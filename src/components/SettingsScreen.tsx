...
import { parseLocaleNumber, formatLocaleCurrency, formatMaskedCurrency, handleCurrencyKeyDown, handleCurrencySelection } from '../lib/currency';
...
export function SettingsScreen({ 
  ...
  const periodLabel = useMemo(() => {
    ...
    if (filter === 'week' && selectedWeek !== undefined && selectedYear !== undefined) {
      ...
      const firstThursday = new Date(selectedYear, 0, 4 + (4 - dayOfWeek));
      ...
      if (monday.getMonth() === sunday.getMonth()) {
        ...
        return `${String(startDay).padStart(2, '0')} a ${String(endDay).padStart(2, '0')} de ${monthFull} de ${selectedYear}`;
      } else {
        return `${String(startDay).padStart(2, '0')} de ${startMonthName} a ${String(endDay).padStart(2, '0')} de ${endMonthName} de ${selectedYear}`;
      }
    }
    return '';
  ...
  React.useEffect(() => {
    ...
    ['maintenance', 'washing', 'fuel', 'food', 'toll', 'parking', 'ferry'].forEach(id => {
  ...
  const [addingSubToPlatformId, setAddingSubToPlatformId] = useState<string | null>(null);
  const [newSubcategoryName, setNewSubcategoryName] = useState('');
  const [newPlatformSubcategoryName, setNewPlatformSubcategoryName] = useState('');
  
  const [editingSubCategoryId, setEditingSubCategoryId] = useState<string | null>(null);
  const [editingSubOriginalName, setEditingSubOriginalName] = useState('');
  const [editingSubName, setEditingSubName] = useState('');

  const [editingSubPlatformId, setEditingSubPlatformId] = useState<string | null>(null);
  const [editingSubPlatformOriginalName, setEditingSubPlatformOriginalName] = useState('');
  const [editingSubPlatformName, setEditingSubPlatformName] = useState('');
  
  const [showSuccess, setShowSuccess] = useState(false);
  const [errorAlert, setErrorAlert] = useState<string | null>(null);
  const [showLangDropdown, setShowLangDropdown] = useState(false);
  const [openFaqIndex, setOpenFaqIndex] = useState<number | null>(null);
  const hasVehicles = profileData.vehicles && profileData.vehicles.length > 0;
  ...
  const handleAddPlatformSubcategory = (platformId: string) => {
    if (!newPlatformSubcategoryName.trim()) return;
    const name = newPlatformSubcategoryName.trim();
    const normalizedName = normalize(name);
    
    // Check for duplicates in current platform
    const plat = platforms.find(p => p.id === platformId);
    if (plat) {
      const subs = plat.subcategories || [];
    ...
    const updated = platforms.map(p => {
      if (p.id === platformId) {
        ...
        return { ...p, subcategories: [...subs, name] };
      }
      return p;
    });
    onSavePlatforms(updated);
    setNewPlatformSubcategoryName('');
    setAddingSubToPlatformId(null);
  };

  const handleDeletePlatformSubcategory = (platformId: string, subName: string) => {
    const updated = platforms.map(p => {
      if (p.id === platformId) {
        return { ...p, subcategories: (p.subcategories || []).filter(s => s !== subName) };
      }
      return p;
    });
    onSavePlatforms(updated);
  ...
  const handleUpdatePlatformSubcategory = () => {
    if (!editingSubPlatformName.trim() || !editingSubPlatformId) return;
    const name = editingSubPlatformName.trim();
    const normalizedName = normalize(name);
    
    // Check for duplicates
    const plat = platforms.find(p => p.id === editingSubPlatformId);
    if (plat) {
      const subs = plat.subcategories || [];
      const exists = subs.some(s => {
        if (s === editingSubPlatformOriginalName) return false;
    ...
    const updated = platforms.map(p => {
      if (p.id === editingSubPlatformId) {
        return { ...p, subcategories: (p.subcategories || []).map(s => s === editingSubPlatformOriginalName ? name : s) };
      }
      return p;
    });
    onSavePlatforms(updated);
    setEditingSubPlatformId(null);
  };
  ...
  return (
    <div className="space-y-10">
      ...
      <div className="grid grid-cols-12 gap-6">
        {/* Profile Section */}
        <section className="col-span-12 lg:col-span-8 lg:col-start-3 bg-surface-container-lowest rounded-xl p-6 shadow-sm">
        ...
        <section className="col-span-12 bg-surface-container-lowest rounded-xl p-8 shadow-sm overflow-hidden border border-outline-variant/10">
          ...
          {hasVehicles ? (
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                  ...
                  <div className="space-y-3">
                    ...
                    <input 
                      ...
                      placeholder="0,00" 
                      value={monthlyInput} 
                      onChange={(e) => setMonthlyInput(formatMaskedCurrency(e.target.value, language))} 
                      onKeyDown={(e) => handleCurrencyKeyDown(e, monthlyInput, setMonthlyInput, language)}
            ...
            <div className="bg-amber-500/5 border border-amber-500/10 p-6 rounded-2xl flex flex-col md:flex-row items-center gap-4 text-amber-600 mt-4">
              <AlertCircle size={24} className="shrink-0" />
              <div className="text-center md:text-left space-y-1">
                <p className="font-black text-sm text-on-surface">
                  {language === 'pt-BR' ? 'Veículo necessário para metas' : 'Vehicle required for goals'}
                </p>
                <p className="text-xs text-on-surface-variant font-medium">
                  {language === 'pt-BR' 
                    ? 'Você precisa cadastrar pelo menos um veículo na aba "Meus Veículos" antes de definir metas.' 
                    : 'You must register at least one vehicle in the "My Vehicles" tab before configuring earning goals.'}
                </p>
              </div>
            </div>
          )}
        ...
        <section className="col-span-12 bg-surface-container-lowest rounded-xl p-8 shadow-sm overflow-hidden border border-outline-variant/10 mt-6">
          ...
          {hasVehicles ? (
                  ...
                  {['maintenance', 'washing', 'fuel', 'food', 'toll', 'parking', 'ferry'].map(id => (
                    <div key={id} className="space-y-3">
                      ...
                      <input 
                        ...
                        onChange={e => setCategoryBudgetsInput(prev => ({...prev, [id]: formatMaskedCurrency(e.target.value, language)}))} 
                        onKeyDown={e => handleCurrencyKeyDown(e, categoryBudgetsInput[id] || '0,00', (val) => setCategoryBudgetsInput(prev => ({...prev, [id]: val})), language)}
                        onFocus={handleCurrencySelection}
                        onClick={handleCurrencySelection}
                      />
                    </div>
            ...
            <div className="bg-amber-500/5 border border-amber-500/10 p-6 rounded-2xl flex flex-col md:flex-row items-center gap-4 text-amber-600 mt-4">
              ...
              <div className="text-center md:text-left space-y-1">
                <p className="font-black text-sm text-on-surface">
                  {language === 'pt-BR' ? 'Veículo necessário para orçamentos' : 'Vehicle required for budgets'}
                </p>
                <p className="text-xs text-on-surface-variant font-medium">
                  {language === 'pt-BR' 
                    ? 'Você precisa cadastrar pelo menos um veículo na aba "Meus Veículos" antes de definir orçamentos.' 
                    : 'You must register at least one vehicle in the "My Vehicles" tab before configuring budgets.'}
                </p>
              </div>
            </div>
          )}
        ...
        <section className="col-span-12 bg-surface-container-lowest rounded-xl p-6 shadow-sm">
          <div className="flex flex-col md:flex-row md:items-center justify-between mb-5 gap-4">
            ...
            <div className="space-y-2 max-h-[400px] overflow-y-auto pr-2 custom-scrollbar">
              {platforms.filter(p => p.type === 'variable').map(plat => (
                  ...
                  <div className="space-y-1 bg-surface-container-low rounded-xl border border-outline-variant/5">
                    <div className="flex items-center justify-between p-2.5 px-4 group">
                      ...
                      <div className="flex items-center gap-0.5">
                        ...
                        <button 
                          onClick={() => setAddingSubToPlatformId(addingSubToPlatformId === plat.id ? null : plat.id)}
                          className="p-1.5 rounded-lg hover:bg-surface-container-high text-primary transition-colors"
                          title={t('newSubcategory')}
                        >
                          <Plus size={16} />
                        ...
                        >
                          <div className="flex gap-2 p-2 bg-surface-container-highest/50 rounded-lg mx-2 border border-outline-variant/10">
                            <input 
                              ...
                              className="flex-1 bg-surface-container-lowest p-2 rounded text-[10px] outline-none"
                              placeholder={t('newSubcategory')}
                              value={newPlatformSubcategoryName}
                              onChange={(e) => setNewPlatformSubcategoryName(e.target.value)}
                              onKeyDown={(e) => e.key === 'Enter' && handleAddPlatformSubcategory(plat.id)}
                              autoFocus
                            />
                            <button 
                              onClick={() => handleAddPlatformSubcategory(plat.id)}
                              className="p-2 bg-primary text-on-primary rounded-lg transition-transform hover:scale-95"
                            >
                              <Plus size={12} />
                            </button>
                            <button 
                              onClick={() => { setAddingSubToPlatformId(null); setNewPlatformSubcategoryName(''); }}
                              className="p-2 text-on-surface-variant hover:bg-surface-container-high rounded-lg"
                            >
                              <X size={12} />
                            </button>
                    ...
                    <AnimatePresence>
                      {expandedPlatforms.has(plat.id) && (
                        ...
                        >
                          <div className="py-1 space-y-1">
                            <div className="divide-y divide-outline-variant/10 bg-transparent overflow-hidden">
                              {(plat.subcategories || []).map(sub => (
                                <div key={sub} className="flex items-center justify-between py-2 px-4 hover:bg-surface-container-low transition-colors rounded-r-lg">
                                  {editingSubPlatformId === plat.id && editingSubPlatformOriginalName === sub ? (
                                    <div className="flex items-center gap-2 w-full">
                                      <input 
                                        type="text" 
                                        className="flex-1 bg-surface-container-lowest text-xs p-1.5 rounded-lg outline-none border border-primary/30"
                                        value={editingSubPlatformName}
                                        onChange={(e) => setEditingSubPlatformName(e.target.value)}
                                        onKeyDown={(e) => e.key === 'Enter' && handleUpdatePlatformSubcategory()}
                                        autoFocus
                                      />
                                      <div className="flex items-center gap-1">
                                        <button onClick={handleUpdatePlatformSubcategory} className="text-primary p-1 hover:bg-primary/10 rounded-md transition-colors"><CheckCircle size={16} /></button>
                                        <button onClick={() => setEditingSubPlatformId(null)} className="text-on-surface-variant p-1 hover:bg-surface-container-high rounded-md transition-colors"><X size={16} /></button>
                                      </div>
                                    </div>
                                  ) : (
                                    <>
                                      <div className="flex items-center gap-2">
                                        <div className="w-1.5 h-1.5 rounded-full bg-on-surface/30" />
                                        <span className="text-xs font-bold text-on-surface-variant">{sub}</span>
                                      </div>
                                      <div className="flex items-center gap-0.5">
                                        <button onClick={() => { setEditingSubPlatformId(plat.id); setEditingSubPlatformOriginalName(sub); setEditingSubPlatformName(sub); }} className="text-on-surface-variant hover:text-primary p-1 rounded-md hover:bg-surface-container-high transition-colors"><Edit2 size={12} /></button>
                                        <button onClick={() => {
                                          handleDeletePlatformSubcategory(plat.id, sub);
                                        }} className="text-on-surface-variant hover:text-error p-1 rounded-md hover:bg-surface-container-high transition-colors"><Trash2 size={12} /></button>
                                      </div>
                                    </>
                                  )}
        ...
        <section className="col-span-12 lg:col-span-8 lg:col-start-3 bg-surface-container-lowest rounded-xl p-6 shadow-sm mt-6">
          <h3 className="text-lg font-bold font-headline mb-4 flex items-center gap-2">
            <Database className="text-primary" size={20} />
            {language === 'pt-BR' ? 'Área de Teste' : 'Testing Area'}
          </h3>
          <p className="text-sm text-on-surface-variant font-body mb-6">
            {language === 'pt-BR' 
              ? 'Popule o sistema com dados de demonstração contendo 3 meses de corridas, gastos com combustível (gasolina e etanol), alimentação e manutenções para testar o desempenho e os gráficos de relatório.' 
              : 'Fill the app with 3 months of simulated rides, fuel expenses (gasoline and ethanol), food, and maintenance to test fuel performance and report charts.'}
          </p>
          ...
          <div className="flex flex-col sm:flex-row gap-4">
            <button
              ...
              className="flex-1 shrink-0 bg-primary text-on-primary hover:bg-primary/95 font-headline font-bold text-sm px-5 py-3 rounded-xl transition duration-200 cursor-pointer flex items-center justify-center gap-2 shadow-sm disabled:opacity-50"
            >
              {isGenerating ? (
                <span>{language === 'pt-BR' ? 'Gerando dados...' : 'Generating data...'}</span>
              ) : (
                <>
                  <Database size={16} />
                  <span>{language === 'pt-BR' ? 'Gerar Dados de Teste (3 Meses)' : 'Generate Test Data (3 Months)'}</span>
                </>
              )}
            </button>
            ...
            <button
              ...
              className={`flex-1 shrink-0 font-headline font-bold text-sm px-5 py-3 rounded-xl transition duration-200 cursor-pointer flex items-center justify-center gap-2 border shadow-sm ${
                isWiping 
                  ? 'bg-red-600 text-white border-red-600 hover:bg-red-700 animate-pulse animate-duration-1000' 
                  : 'bg-surface-container-low text-error border-error/20 hover:bg-error/5 hover:border-error/30'
              }`}
            >
              <Trash2 size={16} />
              <span>
                {isWiping 
                  ? (language === 'pt-BR' ? 'Clique de novo para Confirmar!' : 'Click again to Confirm!')
                  : (language === 'pt-BR' ? 'Limpar Todos os Dados' : 'Clear All Data')}
              </span>
            </button>
          </div>
        </section>
